...
Оба идут с `--no-bail`. Без него `pnpm --parallel` гасит соседей, как только
любой из скриптов вышел с ненулевым кодом: 02.09.2026 остановка одного только
фронтенда унесла и backend, что выглядело как «стенд упал сам». Теперь
выжившая половина продолжает работать, а провалившаяся всё равно громко
названа в итоговой сводке.

**Прогон тестов больше не ломает стенд.** Раньше после `npx jest` каждая
страница отвечала 500 до `rm -rf apps/frontend/.next` и перезапуска. Причина
оказалась не в кэше: `tests/branding.test.cjs` создавал и тут же удалял
файлы-фикстуры прямо в `apps/**` и `libraries/**` — а это маски `content`
в `apps/frontend/tailwind.config.cjs`. Работающий `next dev` перечислял файл,
тот исчезал за миллисекунды, и сборка CSS падала с `ENOENT`. Фикстуры
переехали во временный каталог, а `tests/helpers/source-tree-guard.cjs`
(подключён через `setupFiles`) теперь отказывает любому тесту, который пишет
в `apps/**` или `libraries/**`, и подсказывает, как сделать правильно.
