...
С волны «заготовка и адаптации» (06.09.2026, `content-factory-next-tu3k.9`)
цепочка «бриф → вариант → пост» из этой таблицы наконец правдива в коде:
Content Brief живёт в `ContentPiece.brief`, Content Variant — это
`ContentDerivation` с собственным текстом (`kind`, `title`, `body`), а
`Post` по-прежнему материализуется только через `PostsService`. До неё бриф
выбрасывался после хода, а «вариант» не мог существовать без поста.
