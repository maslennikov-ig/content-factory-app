...
Решение владельца 06.09.2026 (`content-factory-next-tu3k`): вкладка «Бриф» с
восемью полями «слишком сложна» для ежедневного «появилась мысль или скопировал
чужой пост → хочу свой пост». Дверь `POST /content-intelligence/intake` берёт
одно поле и до трёх каналов, бриф заполняет модель, черновик появляется сразу.
Для памяти это ещё один потребитель, и правила у него те же.

Три вида входа, и они различаются без модели (`intake-kind.ts`):

- **мысль** — всё, что не похоже на остальные два. Дешевле всего: разбора
  чужого текста нет;
- **ссылка** — один адрес `https` и не больше сорока знаков всего остального;
- **чужой пост** — от 400 знаков и либо два числа с единицами, либо речь от
  первого лица в двух предложениях; шапка пересылки решает сразу.

Что становится доказательством, а что нет:

- **страница по ссылке становится.** Она читается один раз тем же шлюзом и той
  же политикой доступа, что и `validateSource` (закрепление адреса, запрещённые
  домены, `robots.txt`), и сохраняется через `acceptSearchResult` с
  `provider: 'user_link'` и `reuseBy: 'url'`. Колонка `retrievalProvider`
  получает `search-result-user_link-v1`; это обычная строка, миграции нет. Та
  же ссылка второй раз переиспользует запись, а не заводит вторую;
- **страница, подтвердившая число из чужого поста, становится.** Проверяются
  первые три числовых утверждения, по одному поиску на каждое. Подтверждение
  детерминированное (`claim-match.ts`): числовая лексема утверждения обязана
  совпасть с лексемой выдержки после снятия пробелов всех видов, замены запятой
  на точку и отбрасывания процента и валюты. Сравнение по равенству лексем, а
  не по вхождению: «137» не подтверждает «37»;
- **сам чужой текст не становится ничем.** Он входит ровно в один промпт — в
  огороженный блок разбора, одной строкой, — и дальше не идёт никуда: ни в
  аргументы генератора, ни в подсказки, ни в черновик. Дальше едут тема, угол,
  строение и отрезки по восемь слов, по которым проверяется, что дословного
  заимствования не случилось;
- **непроверенное утверждение не становится.** Оно видно в квитанции строкой
  `ungrounded` и в текст не идёт. Идентификатор факта, которого в этой области
  нет, снимается со строки, а строка остаётся видимой и неподтверждённой:
  молча выброшенное утверждение человек не заметит.

Ворота брифа (`brief-gate.ts`) не менялись. Изменилось то, о чём спрашивают:
тезис и факты спрашиваются всегда, когда их нет; позицию, возражение и адресата
модель предлагает сама, и спрашивают о них только тогда, когда она честно
вернула пустоту и человек не отдал поле ей же («Реши сама»). Вопросов не больше
двух, и они терминальны — черновика в этом ответе нет.

Цена одного входа: не больше двух вызовов модели своей операцией `intake` (роль
`extract`), не больше трёх поисков и не больше трёх генераций. Поиск и
генерация учитываются своими операциями и здесь второй раз не считаются.

Черновик сохраняется тем же путём, что и любой другой пост, —
`ContentBriefRepository.createDraft` → `PostsRepository.createOrUpdatePost` в
состоянии `DRAFT`, — и несёт снимок контекста, версию голоса и метки цитат,
чтобы окно поста показало строку происхождения.

Одно ограничение установки: чтение ссылки соблюдает выключатель
`SOURCE_DIRECT_FETCH`. Он выражает решение «этот сервер не ходит по
произвольным адресам», и обходить его ради удобства значило бы снять
единственный рычаг, которым это решение выражено. Пока он не включён, вход по
ссылке отвечает `INTAKE_LINK_UNREACHABLE` с понятным «вставьте текст поста
прямо в поле»; мысль и чужой пост работают.

### Где живёт бриф заготовки

Волна «заготовка и адаптации» (`content-factory-next-tu3k.9`, 06.09.2026)
перестала выбрасывать заполненный бриф. До неё вход держал его в памяти хода и
записывал по материалу на каждый канал с HTML одного канала в теле. Теперь:

- **заготовка** — `ContentPiece` с `kind = 'CORE'`: `body` — нейтральная суть
  простым текстом, `brief` (Json) — `ZagotovkaCoreV1` без `text`: заполненный
  бриф с происхождением каждого поля, ответы интервью дословно, отчёт
  проверки на штампы, кем написана суть (`model` или `fallback`), принёс ли
  автор своё число. Записывается **один раз до цикла по каналам**;
- **адаптация** — `ContentDerivation` с `kind`, `title`, `body` (текст под
  площадку простым текстом), `mediaId` и, как прежде, связью с постом и версией
  голоса. Ответы на вопросы под канал хранятся при адаптации;
- **состояние публикации не хранится**: читается по связи с постом
  (`published > queued > error > draft`, `deletedAt` — состояния нет) одним
  запросом по индексу `[organizationId, postId]`.

Бриф — не факт памяти. Утверждения из него становятся `ContentFact` тем же
путём, что и раньше, только после подтверждения; строка `ungrounded` в брифе
остаётся видимой и в текст не идёт. Ответ человека на вопрос интервью —
его слово (§9.5 карты раздела): ложится в бриф с происхождением `person`,
без приглаживания, и в суть переносится дословно.

Цена: суть — один вызов роли `draft` операцией `intake`; адаптация — одна
генерация с правилами канала, новых платных операций нет. Отказ модели на сути
не валит вход: суть собирается из брифа детерминированно и помечается
`writtenBy: 'fallback'`.

Старые строки читаются без переноса: материал без `kind` — заготовка, у
которой суть не выделена (`core: null`, тело в `legacyBody`); производная без
`body` берёт текст из поста.
...
| Мысль без фактов, поиск выключен                                          | контекст не строится: до генератора не доходит                        | вход отвечает вопросом про факты, черновика нет (`content-factory-next-tu3k`)                    |
