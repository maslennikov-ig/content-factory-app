...
-- На боевой базе ПОКА НЕ ПРИМЕНЕНО. Применять до переключения на образ с этим
-- деревом: без каскадов новая дверь удаления упадёт на внешнем ключе. Повторно
-- не запускать — `ADD CONSTRAINT` откажет на уже существующем имени.
