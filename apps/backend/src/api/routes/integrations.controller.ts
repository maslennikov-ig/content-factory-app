...
import { IntegrationFunctionDto } from '@contentfactory/nestjs-libraries/dtos/integrations/integration.function.dto';
...
import { TelegramUpdatesDto } from '@contentfactory/nestjs-libraries/dtos/integrations/telegram.updates.dto';
import {
...
export class IntegrationsController {
  ...
  updateContentLanguage(
    @GetOrgFromRequest() org: Organization,
    @Param('id') id: string,
    @Body() body: IntegrationContentLanguageDto
  ) {
    return this._integrationService.updateContentLanguage(
      org.id,
      id,
  ...
  async getIntegrationUrl(
    ...
    try {
      ...
      await ioRedis.set(`product-event-user:${state}`, user.id, 'EX', 3600);
  ...
  async getUpdates(@Query() query: TelegramUpdatesDto) {
    return this._telegramUpdatesService.getConnection(query.word);
  }
