  ...
  theme: {
    extend: {
      ...
      screens: {
        ...
        /**
         * Где список заготовок перестаёт быть карточками и становится таблицей.
         *
         * Именованный экран, а не `min-[720px]:`, и это не вкусовщина. Tailwind
         * 3.4 отключает варианты `min-*`/`max-*` целиком, как только в
         * `screens` появляется хоть один объект с `raw` — а их здесь шесть
         * штук выше. Он не падает и ничего не подчёркивает: пишет
         * `complex-screen-config` в лог сборки и молча не выпускает ни одного
         * правила. Таблица заготовок так и уехала на боевой с
         * `min-[720px]:block`, который в CSS не существовал, и не отрисовалась
         * ни разу. Именованный экран этой поломки не знает — он обычный
         * брейкпоинт, и с `raw`-соседями живёт спокойно.
         *
         * 720 — это `PIECE_TABLE_MIN_WIDTH` из `voice-wiring.contract.ts`;
         * два числа обязаны совпадать.
         */
        table: '720px',
      },
    },
