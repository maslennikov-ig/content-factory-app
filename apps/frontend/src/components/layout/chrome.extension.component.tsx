...
export const ChromeExtensionComponent = () => {
  ...
  const t = useT();
  const label = t('browser_extension', 'Browser extension');

  // Only offered when this deployment actually publishes an extension.
  if (!extensionId) {
    return null;
  }
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="w-[32px] h-[32px] rounded-[8px] flex items-center justify-center hover:bg-cf-surface-subtle hover:text-cf-ink transition-colors duration-state"
    >
      <svg
