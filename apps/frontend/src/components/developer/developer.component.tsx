...
import { MediaBox } from '@contentfactory/frontend/components/media/media.component';
...
import { DeveloperSurface } from './developer.surface';
...
export const DeveloperComponent: FC = () => {
  ...
  const createApp = useCallback(async () => {
    if (!name || !redirectUrl) {
      toaster.show(
        t(
          'oauth_name_and_redirect_required',
    ...
    try {
      ...
      toaster.show(
        t('oauth_app_create_failed', 'Failed to create app'),
        'warning'
      );
    }
  ...
  const updateApp = useCallback(async () => {
    try {
      ...
      toaster.show(
        t('oauth_app_update_failed', 'Failed to update app'),
        'warning'
      );
    }
  }, [name, description, redirectUrl, pictureId]);
  ...
  const rotateSecret = useCallback(async () => {
    ...
    try {
      ...
      toaster.show(
        t('oauth_secret_rotate_failed', 'Failed to rotate secret'),
        'warning'
      );
    }
  }, [decision]);
  ...
  const deleteApp = useCallback(async () => {
    ...
    try {
      ...
      toaster.show(
        t('oauth_app_delete_failed', 'Failed to delete app'),
        'warning'
      );
    }
  }, [decision]);
  ...
  if (app === undefined) {
    return <DeveloperSurface state="loading" />;
  }

  // No app yet — show create prompt
  if (!app && !creating) {
    return (
      <DeveloperSurface state="empty">
      <div className="flex flex-col gap-[40px]">
        <div className="text-[14px] text-textColor leading-[1.7]">
          {t(
            'oauth_app_note_line1',
            'Create an OAuth App to let other Content Factory users authorize your product to post on their behalf.'
          )}
          <br />
          {t(
        ...
        <div className="bg-newBgColorInner rounded-[12px] border border-newBorder overflow-hidden">
            ...
            <div>
              ...
              <div className="text-[13px] text-customColor18 mt-[2px]">
                {t(
                  ...
                  'Create an OAuth application to allow third-party integrations with Content Factory on behalf of your users.'
                )}
              </div>
            </div>
            <div className="flex gap-[6px] shrink-0 pt-[2px]">
              <DocsLink path="/public-api/oauth" />
            </div>
          </div>
          <div className="p-[20px]">
            <Button
              ...
              className="cursor-pointer px-[20px] transition-colors rounded-[8px] text-[15px] font-[600]"
            >
  ...
  if (creating && !app) {
    return (
      <DeveloperSurface state="selected">
      <div className="flex flex-col gap-[40px]">
        <div className="text-[14px] text-textColor leading-[1.7]">
          {t(
            'oauth_app_note_line1',
          ...
          <div className="p-[20px] flex flex-col gap-[16px]">
            <div className="flex flex-col gap-[6px]">
              ...
              <Input
                ...
                placeholder={t('developer_app_name_placeholder', 'My Application')}
                maxLength={100}
              />
            </div>
            <div className="flex flex-col gap-[6px]">
              ...
              <Textarea standalone
                layout="content"
                className="bg-newBgColorInner border border-newBorder rounded-[8px] p-[16px] text-textColor outline-none"
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder={t(
                  'developer_app_description_placeholder',
            ...
            <div className="flex flex-col gap-[6px]">
              ...
              <div className="flex items-center gap-[12px]">
                {picturePath ? (
                  <img
                    src={picturePath}
                    alt={t('developer_app_picture_alt', 'App picture')}
                ...
                <Button variant="secondary"
                  type="button"
                  onClick={openMedia}
                  className="cursor-pointer px-[16px] transition-colors rounded-[8px] text-[13px] font-[600]"
                >
  ...
  return (
    <DeveloperSurface state={editing ? 'selected' : plaintextSecret ? 'success' : 'default'} name={app.name} description={app.description}>
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        {t(
          'oauth_app_note_line1',
