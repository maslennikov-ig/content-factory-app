...
const ConnectedComponent: FC<{
  ...
  const disconnect = useCallback(async () => {
    if (
      !(await deleteDialog(
        t(
          'github_disconnect_repository_confirm',
...
const ConnectComponent: FC<{
  ...
  const completeConnection = useCallback(async () => {
    ...
    if (response.status === 404) {
      toast.show(t('github_repository_not_found', 'Repository not found'), 'warning');
      return;
    }
  ...
  return (
    <div className="my-[16px] flex min-h-[100px] rounded-[8px] border border-cf-border bg-cf-surface px-[24px]">
      <div className={`flex items-center gap-[8px] flex-1`}>
        ...
        <Button variant="quiet" className="mt-[7px]" onClick={cancelConnection}>
          {t('cancel', 'Cancel')}
        </Button>
        <Input
          value={url}
