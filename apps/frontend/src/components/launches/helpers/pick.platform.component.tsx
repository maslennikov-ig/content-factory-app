...
import { useMoveToIntegrationListener } from '@contentfactory/frontend/components/launches/helpers/use.move.to.integration';
                  ...
                  !props.singleSelect ? (
                    ...
                    >
                      <div
                        ...
                        className={clsx(
                          'cursor-pointer relative w-[48px] h-[48px] rounded-full flex justify-center items-center bg-cf-surface-subtle transition-all duration-state',
                    ...
                    <div key={integration.id} className="">
                      <div
                        onClick={addPlatform(integration)}
                        className={clsx(
                          'cursor-pointer rounded-full w-[200px] relative h-[40px] flex justify-center items-center transition-all duration-state',
                          selectedAccounts.findIndex(
                            (p) => p.id === integration.id
                          ) === -1
                            ? 'bg-cf-surface border border-cf-border-control text-cf-ink'
                            : 'bg-cf-accent-soft border border-cf-accent text-cf-ink'
                        )}
                      >
                        <div className="flex items-center justify-center gap-[8px]">
                          <div className="relative">
                            ...
                            {/*
                              Inside a 40px pill that its own scroller clips at
                              40px, so the badge sits within the avatar instead
                              of hanging off it: 32 + 4 of overhang would put
                              its lower edge past the box.
                            */}
                            <PlatformBadge
                              identifier={integration.identifier}
                              size={16}
