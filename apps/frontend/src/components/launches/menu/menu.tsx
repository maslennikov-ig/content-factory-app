...
import { TimeTable } from '@contentfactory/frontend/components/launches/time.table';
import {
  Integrations,
  useCalendar,
} from '@contentfactory/frontend/components/launches/calendar.context';
import { BotPicture } from '@contentfactory/frontend/components/launches/bot.picture';
import { CustomerModal } from '@contentfactory/frontend/components/launches/customer.modal';
import { Integration } from '@prisma/client';
import { SettingsModal } from '@contentfactory/frontend/components/launches/settings.modal';
import { CustomVariables } from '@contentfactory/frontend/components/launches/add.provider.component';
...
export const Menu: FC<{
   ...
   * канала задаёт оба (`content-factory-next-fn33.90.4`, `.90.5`).
   *
   * Всё, что лежит под `/integrations/:id/*` — настройки, группа, временные
   * интервалы, имя и картинка бота, переподключение, — с 03.09 и 05.09.2026
   * несёт `Sections.ADMIN`: канал публикует от имени пространства, значит это
   * его общее имущество. «Создать пост» ведёт к дверям `/posts`, а они с
   * 05.09.2026 несут `Sections.EDITOR`.
   *
   * До этой правки роль проверяли только отключение, включение и удаление, а
   * остальные семь пунктов открывались всем. Живой прогон 05.09 прошёл их под
   * Пользователем и под Редактором: форма открывалась целиком, человек её
   * заполнял, и «Сохранить» отвечало 403. Обещать форму, которую некому
   * сохранить, — та же ошибка, что обещать каталог провайдеров.
   *
   * Обе проверки — те же функции, что читает сервер в `permissions.service.ts`
   * (`docs/product/roles-matrix.md`). Второго мнения о том, кто такой
   * редактор, здесь нет.
   */
  ...
  const additionalSettings = useCallback(() => {
    ...
    modal.openModal({
      title: t('channel_settings', 'Channel settings'),
      children: (
        <SettingsModal
          // @ts-ignore
          integration={findIntegration}
      ...
      */}
      ...
      {show && (
        ...
        >
          ...
          {manageChannel && !!findIntegration?.isCustomFields && (
            <MenuAction
          ...
          {manageChannel && (canChangeProfilePicture || canChangeNickName) && (
            <MenuAction
