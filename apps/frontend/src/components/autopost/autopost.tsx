...
const details = object().shape({
  ...
  researchEnabled: boolean().required(),
  language: string().oneOf(['en', 'ru']).required(),
...
export const AddOrEditWebhook: FC<{
  ...
  const languageWasSelected = useRef(false);
  ...
  const form = useForm({
    ...
    values: {
      ...
      language: data?.language || (interfaceLanguage === 'ru' ? 'ru' : 'en'),
  ...
  useEffect(() => {
    if (data?.language || languageWasSelected.current || !dataList) {
      return;
    }
    const channels =
      allIntegrations.value === 'all'
        ? dataList.integrations
        : (integrations as Array<{ contentLanguage?: string }>);
    form.setValue(
      'language',
      defaultContentLanguageForIntegrations(
        channels || [],
        form.getValues('language') === 'ru' ? 'ru' : 'en'
      ),
      { shouldValidate: true }
    );
  }, [allIntegrations.value, data?.language, dataList, form, integrations]);
        ...
        <div className="relative flex gap-[20px] flex-col flex-1 rounded-[4px] border border-customColor6 pt-0">
          <div>
            ...
            {generateContent && (
              <Select
                label={t(
                  'autopost_web_research',
                  'Enrich with web research (adds search cost)'
                )}
                {...form.register('researchEnabled', {
                  setValueAs: (value) => value === 'true' || value === true,
                })}
              >
                {optionsChoose.map((option) => (
