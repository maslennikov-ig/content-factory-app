      ...
      webhooks.map(async (webhook) => {
        try {
          await fetch(webhook.url, {
            ...
            // The stored URL was validated once, when it was saved; delivery
            // happens much later and re-resolves DNS. The dispatcher checks
            // every resolved IP at connect time, and `manual` stops a 302 from
            // carrying the post to an address nothing validated.
            redirect: 'manual',
