...
public_facade: n/a
bounded_acceptance: pnpm exec jest tests/brand-voice.brief-tab.test.cjs
non_goals:
  - переработка раздела за пределами названной беды
  - изменение контрактов дверей и схемы базы
evidence:
  - none
task_id: content-factory-next-fn33.69
...
write_zone:
  ...
  - apps/frontend/src/components/brand-voice/voice-brief.screen.tsx
  - apps/frontend/src/components/brand-voice/voice-brief.adapter.ts
  - tests/brand-voice.brief-tab.test.cjs
success_criteria:
  - Отказ «нет ни одного канала» стоит у нажатой кнопки и ведёт дальше
selected_docs:
...
docs_review_notes: Роль двери не менялась; менялось место и полнота отказа.
verification:
...
Отказ печатался наверху панели, в блоке «Радар тем», а кнопка «Создать черновик» стоит внизу. Отказ этой кнопки отделён от общего (`draftFailure`) и печатается рядом с ней; для `BRIEF_CHANNEL_UNKNOWN` рядом стоит ссылка на «Календарь», где канал подключают. Код читается с самой ошибки: `BRIEF_CHANNEL_UNKNOWN` не входит в `VOICE_ERROR_CODES`, а учить эту общую таблицу дверью брифа значило бы менять контракт, который читают ещё две поверхности.
