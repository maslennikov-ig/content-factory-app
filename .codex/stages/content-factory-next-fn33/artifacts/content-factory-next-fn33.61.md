...
evidence:
  - none
task_id: content-factory-next-fn33.61
...
success_criteria:
  - Пустая витрина фактов называет вкладку «Бриф»
selected_docs:
...
docs_review_notes: Полоса вкладок не менялась, менялся текст о ней.
verification:
...
changed_files:
  ...
  - tests/content-section-tabs.boundary.guard.test.cjs
explicit_defers:
  - Тот же мёртвый ярлык «Что пишем» остался в `apps/frontend/src/components/onboarding/onboarding.copy.ts:106` — файл вне зоны записи потока.
---

# Summary

Две русские строки витрины «Откуда факты» звали во вкладку «Что пишем», которой на полосе нет. Английские строки всё это время называли «Бриф» правильно. Заодно поставлен страж: копия раздела не может назвать вкладку, ушедшую с полосы.
