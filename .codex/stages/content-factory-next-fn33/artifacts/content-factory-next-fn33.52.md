...
public_facade: n/a
bounded_acceptance: pnpm exec jest tests/content-lead-service-recovery.guard.test.cjs
non_goals:
  - переработка раздела за пределами названной беды
  - изменение контрактов дверей и схемы базы
evidence:
  - none
task_id: content-factory-next-fn33.52
...
success_criteria:
  - Отказанная проверка ленты больше не пишет дату чтения
selected_docs:
...
docs_review_notes: Поведение двери не менялось, только запись поля.
verification:
  - pnpm exec jest tests/content-lead-service-recovery.guard.test.cjs: passed
...
При выключенной проверке `checkSubscription` писал `lastCheckedAt: now`, и строка подписки начинала говорить «заглядывали <дата>» о чтении, которого не было. Оба отказных пути (`feedCheckEnabled === false` и `result.disabled`) больше не трогают дату; код отказа `CHECK_DISABLED` по-прежнему пишется. Неудачная проверка — другой случай: лента открывалась, дата ставится.
