...
write_zone:
  ...
  - tests/**
success_criteria:
  - The invitation form asks for a role in the reader’s words
selected_docs:
...
The message was not the server's: this form runs `classValidatorResolver(AddTeamMemberDto)` in the browser, and the raw constraint text — enum values and all — was rendered straight into the field. The shared helper replaces it, keyed on the property name the message opens with. The email field on the same form is routed through the helper as well, for the same reason.
