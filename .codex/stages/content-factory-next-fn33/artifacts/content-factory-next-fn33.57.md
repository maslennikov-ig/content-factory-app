...
evidence:
  - none
task_id: content-factory-next-fn33.57
...
success_criteria:
  - Форма факта спрашивает утверждение, остальное заполняет сама
selected_docs:
...
docs_review_notes: Карта раздела §3/§4 уже описывает это решение; правка приводит экран к ней.
verification:
...
changed_files:
  ...
  - tests/content-intelligence.facts-door.test.cjs
explicit_defers:
  - Автоключ снижает шанс, что два факта об одной теме попадут в одну «тему» радара: ключ теперь из слов фразы, а не из общего словаря. Владельцу это стоит подтвердить.
---

# Summary

Форма открывалась внутренним ключом, окнами свежести и интервалом действия — тем аппаратом, который решением владельца от 01.09.2026 (карта раздела, §3, §4) уходит из интерфейса. Теперь наверху одно поле «Утверждение»; ключ собирается из его слов (`claimKeyFromStatement`), «Значение» по умолчанию берёт само утверждение, а ключ, значение, язык, тип во времени и три даты живут под «Подробнее» — тем же нативным `details/summary`, что панель контекста в `new-launch/editor.tsx`.
