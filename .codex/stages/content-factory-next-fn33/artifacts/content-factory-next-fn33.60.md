...
public_facade: n/a
bounded_acceptance: pnpm exec jest tests/content-section.route.test.cjs
non_goals:
  - переработка раздела за пределами названной беды
  - изменение контрактов дверей и схемы базы
evidence:
  - none
task_id: content-factory-next-fn33.60
...
success_criteria:
  - Адрес идёт за вкладкой раздела «Контент»
selected_docs:
...
docs_review_notes: Список вкладок и `resolveContentTab` не менялись.
verification:
  - pnpm exec jest tests/content-section.route.test.cjs: passed
changed_files:
  - apps/frontend/src/components/content-intelligence/content-section.screen.tsx
  - tests/content-section.route.test.cjs
explicit_defers:
  - Вид «Что уже написали» внутри «Материалов» в адрес не пишется: `?tab=archive` по-прежнему только читается. Отдельный вопрос, за пределами беды.
---

# Summary

Вкладка была только состоянием компонента: «Взять в работу» уводило на «Бриф», а в адресе оставалось `?tab=leads`, и перезагрузка возвращала на «Откуда идеи». Один путь смены вкладки для всех — полоса и повод, — и `history.replaceState` тем же приёмом, что уже применяет `launches/calendar.context.tsx`.
