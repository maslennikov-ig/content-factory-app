...
evidence:
  - none
task_id: content-factory-next-fn33.62
...
success_criteria:
  - Ошибка ключа называет помеху, а не повторяет подсказку
selected_docs:
...
docs_review_notes: Текст поля, контракт двери не менялся.
verification:
...
Красная строка под «Ключ утверждения» дословно повторяла серую подсказку и не говорила, что не так. `claimKeyIssue` различает пробел и отсутствие прямой черты; текст называет помеху («пробелов быть не может: соедините слова знаком подчёркивания»). Пустой ключ ошибкой больше не считается — его заполняет fn33.57.
