...
evidence:
  - none
task_id: content-factory-next-fn33.68
...
success_criteria:
  - Сохранённый факт действительно ложится в бриф строкой
selected_docs:
...
docs_review_notes: Обещание из §9 карты выполняется; менять карту не потребовалось.
verification:
...
Перенос был написан (`onFactCreated` → `handleFactCreated`), но стоял последней строкой обработчика — за привязкой доказательства и обновлением списка. Любой отказ этих двух шагов уводил выполнение в `catch`, и обещание рвалось, а факт при этом оставался на сервере. Теперь перенос идёт сразу после того, как известен id, а обновление списка получило свой `try`, чтобы не печатать «Факт не сохранился» над сохранённым фактом. Проверено в jsdom: без правки строка брифа оставалась пустой.
