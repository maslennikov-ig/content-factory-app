...
## Tenth walk: cnt-28 and the fourth fix wave of the day (22.09.2026) — RELEASED `97923762a35e`

The owner's next piece failed with `AI_INCLUDED_QUOTA_EXHAUSTED`: the instance row said 45 operations a month (the env said 50; the row wins), the owner's workspace had spent 45 since its period start on 16.09. On «Сделай для меня безлимит» the row was set to 1 000 000 (the DTO ceiling) via psql; 97dq.27 records the product tail (a «no limit» state instead of a counter).

Then cnt-28 (evidence/walk-2026-09-22/pieces/cnt-28.json): the owner pasted an editor's messages with three links, his own «Хочу написать у себя в ТГ-канале пост о том, что я выступил на радио… сохранить вот эти ссылки», and the show's announcement with four questions. `detectInputKind` took the first-person sentences as the sign of a reprinted post → `foreign_post`; `personText` empty; the position question «согласен с автором» on his own post; the core an essay on the questions' topic without the appearance or the links. Owner: «Модель совершенно не поняла задание… стоит выбирать тип материала… по умолчанию свой материал… в инструкции ссылки нужно просто сохранить, а не по ним ходить»; and «нет возможности удалить заготовку… из списка и из самой заготовки». Beads 97dq.28, .29, .30 (P1).

- Root, contract + intake: `IntakeInputKindV1` gains `instruction`; the checkbox «Это чужой текст» becomes a three-way `Segmented` above the field («Свой текст» default, «Чужой пост», «Задание»), always sent as an explicit kind — the server never raises a named kind; the first-person heuristic is deleted; the number heuristic strips links and no longer counts «2. Бизнес» list numerals. Instruction: no link reading, no extraction, brief by `intake.prompts.v7.ts` (task, not text; links are addresses to keep; no author position), core by `core-write-prompt.v10.ts` (v9 imported + `CORE_WRITE_INSTRUCTION_V10` + block titles «ЗАДАНИЕ» / «ССЫЛКИ ИЗ ЗАДАНИЯ»); `instructionText`/`keepLinks` stored on the core, carried through answer/enrich, shown as «Ваше задание» above the questions; `keepLinks` handed to the adaptation hints and named in `channelInstructionLines` as an override of the link policy.
- Root, delete: `DELETE /content-intelligence/pieces/:id` (EDITOR, org-scoped `deleteMany` of derivations then piece; posts untouched), `piece-delete.button.tsx` — one armed two-press button used on the piece page and in the expanded list row; roles matrix row updated.
- Root, logging: the intake stream's catch now logs the cause — the first stand run failed with `INTAKE_FAILED` and the server said nothing.
- Live stand run12–run14: instruction core 1125 chars / 3 paragraphs / all three links, Telegram adaptation 1193 chars / all three links (1 of 3 before the directive), foreign post unchanged (823 chars). README fourth pass.

Proof: receipt Jest 457/6185, Node 125/0, Python OK for `6276ea90db23`; tsc × 4; process verification OK; image checks (nginx, no `.env`, commands `--help`, weight 0%); host healthy, five addresses 200, archive byte-equal, Mastra 29→29. Evidence `evidence/release-2026-09-22-fourth.json`. Rollback `7bc2f3016f49`.
