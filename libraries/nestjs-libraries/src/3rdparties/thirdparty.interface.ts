...
export abstract class ThirdPartyAbstract<T = any> {
  ...
  // Dynamically dispatched by ThirdPartyController.callFunction. The
  // organization travels with the call because a provider may reach an AI
  // client, and those resolve their key per organization.
  [key: string]:
    | ((apiKey: string, data: any, organizationId: string) => Promise<any>)
    | undefined;
}
