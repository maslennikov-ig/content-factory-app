...
import { AllProvidersSettings } from '@contentfactory/nestjs-libraries/dtos/posts/providers-settings/all.providers.settings';
import { Integration } from '@prisma/client';
import { checkAuth } from '@contentfactory/nestjs-libraries/chat/auth.context';
import {
  ValidUrlExtension,
  ValidUrlPath,
} from '@contentfactory/helpers/utils/valid.url.path';
...
`,
                ...
                .array(
                  z.object({
                    ...
                    citationIds: z
                      ...
                      .default([])
                      .describe(
                        'Citation ids from the server content context used by this item'
                      ),
                  })
                )
                .describe(
      ...
      execute: async (inputData, context) => {
        ...
        const requestContext = context?.requestContext as any;
        const contentIntelligenceMode =
          requestContext?.get('contentIntelligenceMode') ===
          'content-intelligence/v1';
        let serverContext: any;
        let allowedCitationIds = new Set<string>();
        if (contentIntelligenceMode) {
          try {
            serverContext = JSON.parse(
              (requestContext?.get('contentContext') as string) || '{}'
            );
          } catch {
            serverContext = {};
          }
          if (
            serverContext.contractVersion !== 'content-context/v1' ||
            typeof serverContext.contentContextSnapshotId !== 'string' ||
            serverContext.generationPolicy === 'EVIDENCE_REQUIRED'
          ) {
            throw new Error('A valid server content context is required.');
          }
          if (
            inputData.socialPost.some(
              (post: { type: string }) => post.type !== 'draft'
            )
          ) {
            return {
          ...
          allowedCitationIds = new Set([
            ...(serverContext.facts || []).map((item: any) => item.citationId),
            ...(serverContext.evidence || []).map(
              (item: any) => item.citationId
            ),
          ]);
        }
        const finalOutput = [];
        ...
        for (const post of inputData.socialPost) {
          ...
          if (contentIntelligenceMode) {
            const requestedCitationIds = post.postsAndComments.flatMap(
              (item: any) => item.citationIds || []
            );
            if (
              requestedCitationIds.some(
                (citationId: string) => !allowedCitationIds.has(citationId)
              )
            ) {
              return { errors: 'Unknown content-context citation id.' };
            }
          }
