...
import { AnalyticsData } from '@contentfactory/nestjs-libraries/integrations/social/social.integrations.interface';
...
import { stripLinks } from '@contentfactory/helpers/utils/strip.links';
import { validate } from 'class-validator';
import { plainToInstance } from 'class-transformer';
import { stripHtmlValidation } from '@contentfactory/helpers/utils/strip.html.validation';
import { weightedLength } from '@contentfactory/helpers/utils/count.length';
import { TelegramUpdatesService } from '@contentfactory/nestjs-libraries/integrations/telegram.updates.service';
/**
 * Только тип: значение приходит по имени.
 *
 * Этот сервис поднимается и там, где голосового модуля нет, а `import type`
 * исчезает при компиляции — то есть не тянет за собой ни одного файла раздела
 * «Контент». Имя объявлено в `voice-edit.repository.ts` как `VOICE_EDIT_PORT`,
 * и `tests/brand-voice.edits.test.cjs` следит, чтобы обе стороны звали его
 * одинаково.
 */
...
export class PostsService {
  ...
  constructor(
    ...
    /**
     * Что человек исправил в черновике продукта.
     *
     * `@Optional()`, потому что этот сервис поднимается и там, где раздела
     * «Контент» нет — воркер публикации собирает его же. Отсутствие записи не
     * должно ломать публикацию: правка это материал для будущего замера, а не
     * часть отправки поста.
     */
    @Optional()
    @Inject('VOICE_EDIT_REPOSITORY')
    private _voiceEdits?: VoiceEditRepository
  ) {}
  ...
  async getStatistics(orgId: string, id: string) {
    ...
    const providerIdentifier = getPost[0]?.integration?.providerIdentifier;
    // A provider that is no longer registered — a retired network, a stored
    // identifier from an older release — means "no analytics", not a crash on
    // the statistics screen.
    const hasPostAnalytics = providerIdentifier
      ? !!this._integrationManager.getSocialIntegration(providerIdentifier)
          ?.postAnalytics
      : false;
    const telegramPost = getPost.find(
      (post) =>
        post.integration?.providerIdentifier === 'telegram' && post.releaseId
    );
    const telegram = telegramPost
      ? (await this._telegramUpdatesService.getPostMetrics(
          telegramPost.integration!.token,
          telegramPost.releaseId!
        )) || {
          reactions: 0,
          comments: 0,
          collectedAt: null,
  ...
  async createPost(
    ...
    for (const post of body.posts) {
      ...
      /**
       * Что человек исправил в том, что предложил продукт.
       *
       * После сохранения и в стороне от него: правка это материал для будущего
       * замера порога, и пост, который не удалось записать в эту таблицу,
       * обязан быть отправлен как ни в чём не бывало. Ни на генерацию, ни на
       * порог она пока не влияет — `content-factory-next-pl1.32` держит этот
       * порядок намеренно.
       */
      this.recordVoiceEdit(orgId, posts[0].id, post.value).catch(() => {});
  ...
  /**
   * Пара «что предложили — что отправили», одной строкой на сохранение.
   *
   * Ветка склеивается так же, как её меряет проверка текста над формой: один
   * черновик, один отправленный текст, одно наблюдение. Разложив ветку на
   * куски, продукт посчитал бы длинный пост за пять коротких.
   */
  private async recordVoiceEdit(
    orgId: string,
    postId: string,
    value: CreatePostDto['posts'][number]['value']
  ): Promise<void> {
    if (!this._voiceEdits) return;
    const sent = (value || [])
      .map((one) => (one.content || '').trim())
      .filter(Boolean)
      .join('\n\n');
    if (!sent) return;
    await this._voiceEdits.recordFromPost(orgId, postId, sent);
  }

  async separatePosts(orgId: string, content: string, len: number) {
    return this._openaiService.separatePosts(orgId, content, len);
  }
  ...
  async getProductionAnalytics(
    ...
    const { from, to } = productionAnalyticsWindow(days, now);
    const posts = await this._postRepository.getProductionAnalyticsPosts(
      orgId,
      from,
      to,
      integrationId
