...
import {
  ...
  Video,
  VideoAbstract,
} from '@contentfactory/nestjs-libraries/videos/video.interface';
...
@Video({
  ...
  available:
    ...
    // The AI key is per organization and this metadata is evaluated at import
    // time, so it cannot be checked here. process() raises
    // AiProviderNotConfigured for an organization that has not set one.
    !!process.env.FAL_KEY,
})
