...
/**
 * Вид входа, и с 22.09.2026 его называет человек, а не угадывает продукт
 * (`content-factory-next-97dq.28`, `.29`).
 *
 * `thought` — свои слова как материал: правится речь, слова остаются его.
 * `foreign_post` — чужой текст: свой пост из чужого, вопрос о позиции.
 * `link` — голая ссылка: страница читается, авторство чужое.
 * `instruction` — задание: человек описывает, какой пост нужен. Его слова —
 * не материал и в текст дословно не переносятся; ссылки из задания
 * сохраняются в тексте как есть, и по ним никто не ходит. Появился после
 * десятого захода: «пост о том, что я выступил на радио, вот ссылки, вот
 * вопросы» ушёл как чужой пост и стал эссе по теме вопросов (`cnt-28`).
 */
export type IntakeInputKindV1 = 'thought' | 'link' | 'foreign_post' | 'instruction';
...
export type IntakeQuestionV1 = {
  ...
  options?: string[];
  /**
   * Вариант из `options`, который не ответ, а просьба сказать своими словами:
   * «согласен частично и хочу уточнить свою позицию». Выбор такого варианта
   * открывает поле, а его подпись ответом не становится никогда.
   *
   * `content-factory-next-97dq.23`, девятый заход 22.09.2026: без пометки
   * третий вариант ушёл в бриф дословно (`position`, origin `person`), и суть
   * написалась с фразы кнопки вместо позиции человека. Пометка приходит с
   * сервера именно потому, что узнавать её по тексту на клиенте — это
   * сравнение строк, которое разъедется на первом же переводе.
   */
  ownOption?: string;
  /**
...
export type PieceCellV1 = {
  ...
  more?: number;
  /**
   * Черновик держит время канала «в плане» (`97dq.57`): слово клетки — «в
   * плане», а `date` — это время.
   */
  planned?: boolean;
};
...
export type PieceFieldAnswerV1 = {
  ...
  question?: string;
  /**
   * У `origin: 'person'` — дословно, как написал человек. У `origin: 'model'`
   * (`97dq.56`) — текст решения по отданному вопросу, если оно было, иначе
   * пусто; в промпт такое решение идёт своим блоком, а не как слова человека.
   */
  text: string;
 ...
 */
/**
 * Откуда взялась заготовка, когда она началась не с пустого поля
 * (`content-factory-next-75xn.8`).
 *
 * До этой волны `sourceLeadId` доходил до `IntakePlanV1` и умирал там: ни одна
 * строка не читала его дальше, и принятый повод терял ссылку на материал,
 * из которого вырос. Человек видел заготовку и не имел способа вернуться к
 * источнику — а именно ради источника он повод и взял.
 *
 * Один адрес и один заголовок, не копия повода: очередь поводов остаётся
 * хозяйкой своих строк, а заготовка несёт ровно то, что нужно, чтобы
 * открыть исходное и назвать его.
 */
export type PieceLeadSourceV1 = {
  /** `ContentLead.id`. Хранится, чтобы связь читалась и без адреса. */
  leadId: string;
  /** Адрес материала, из которого вырос повод. */
  url: string;
  /** Заголовок повода, как его показывали в очереди. */
  title?: string;
};
...
export type ZagotovkaCoreV1 = {
  ...
  personText?: string;
  /**
   * Что человек прислал, когда это не его слова: вставленный чужой пост или
   * сообщение со ссылкой, дословно (`content-factory-next-97dq.25`).
   *
   * Нужно странице заготовки, пока сути нет: вопросы задаются по этому
   * тексту, и десятый заход 22.09.2026 показал, что отвечать на них, не видя
   * текста, неудобно. В промпт сути этот текст не идёт ни одним полем — туда
   * едут только пересказанные блоки разбора (`borrowed`), и антикопия
   * держится именно на этом. У собственной мысли здесь пусто: её слова —
   * `personText`.
   */
  sourceText?: string;
  /**
   * Задание человека дословно, когда вход — `instruction`
   * (`content-factory-next-97dq.29`). Странице заготовки — чтобы показать его
   * над вопросами; переписи сути после ответа — чтобы писать по нему снова.
   * В промпт идёт своим блоком «ЗАДАНИЕ», а не «СЛОВА ЧЕЛОВЕКА»: это описание
   * поста, а не его текст, и дословно из него переносятся только ссылки.
   */
  instructionText?: string;
  /**
   * Ссылки из задания, которые велено сохранить: в суть и в адаптацию едут
   * как есть, без чтения страниц. Пусто у всех прочих видов входа.
   */
  keepLinks?: string[];
  /**
   ...
   *
   * Имя не `origin`: `PieceOriginV1` ниже — это род входа, которым библиотека
   * подписывает строку («мысль», «ссылка», «повод»), и два разных смысла под
...
export type AdaptationV1 = {
  ...
  answers?: PieceAnswerV1[];
  /**
   * Квитанция проверок: у свежей адаптации и на странице заготовки.
   *
   * Не хранится нигде — считается заново по телу (`adaptation-checks.ts`,
   * `content-factory-next-97dq.2`), поэтому переживает перезагрузку. В списке
   * заготовок её нет: строка качества там не показывается, и считать её на
   * каждую строку области значило бы платить за невидимое.
   */
  checks?: AdaptationChecksV1;
  /**
   * Как версия стоит в календаре канала (`content-factory-next-97dq.57`).
   * Нет у опубликованной и ошибочной версии и у старого сервера.
   */
 ...
 *
 * `reserved` — держит время канала «в плане на …» и выйдет только после
 * «Запланировать»; `queued` — в очереди (`autopilot` — поставил автопилот);
 * `draft` — черновик без плана: канал «Без плана» или прежняя версия.
 * `current` — эта версия держит слот своей заготовки в канале; черновики
 * прежних версий календарь не показывает. `note` — почему автопилот не
 * поставил версию в очередь.
 */
export type AdaptationPlanV1 = {
  status: 'reserved' | 'queued' | 'draft';
  /** ISO — время слота у `reserved` и `queued`. */
  date: string | null;
  autopilot: boolean;
  current: boolean;
  note?: string | null;
...
/** Режим плана канала: «Без плана», «Бронь» (умолчание), «Автопилот». */
export type ChannelPlanModeV1 = 'draft' | 'reserve' | 'autopilot';
...
export const PIECE_ROUTES = {
  ...
  /**
   * Удалить заготовку насовсем (`content-factory-next-97dq.30`): строка и её
   * адаптации уходят, посты в каналах — включая опубликованные — остаются.
   */
  delete: {
    method: 'DELETE',
    path: (pieceId: string) => `${PIECES_API_BASE}/${pieceId}`,
