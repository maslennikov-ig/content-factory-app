 ...
 * отличить «сюда ещё не писали» от «сюда писать нечем» без списка каналов
 * нельзя. Комментарий над ней это обещал — и до 07.09.2026 никто этого не
 * делал, так что состояние `no_channel` существовало в контракте, в словах
 * экрана и в клетке, но в ответе не появлялось ни разу.
 *
 * Поднимается ровно `none` и ровно при `channels === 0`. Опубликованное,
 * запланированное, черновик и «не ушло» остаются собой при любом числе
 * каналов: то, что уже вышло, не перестаёт существовать оттого, что канал
 * отвязали, — тот же довод, по которому такая площадка остаётся колонкой.
 * `unknown` тоже не трогается: «публикации ещё не прочитаны» — это незнание,
 * а не отсутствие канала, и подменять одно другим значит соврать увереннее,
 * чем есть основания.
 */
export function promoteNoChannel(
  cell: PieceCellV1,
  column: Pick<PieceColumnV1, 'channels'> | undefined
): PieceCellV1 {
  if (cell.state !== 'none') return cell;
  if (!column || column.channels > 0) return cell;
  return { ...cell, state: 'no_channel' };
}
