...
/*
  Шесть групп ниже стояли прямо внутри своих правил и вынесены сюда без единого
  изменения слова (`content-factory-next-tu3k.9.3`). Причина не в стиле: суть
  заготовки пишется промптом, который перечисляет запрещённые обороты, и брать
  их он обязан из этого каталога, а не из второго списка рядом. Второй список
  разошёлся бы с первым на первой же правке — ровно так и появляется правило,
  которое проверка ловит, а модель по-прежнему пишет.
*/

const PARTICIPLE_CLICHE = [
  'подчёркивая',
  'демонстрируя',
  'свидетельствуя',
  'способствуя',
 ...
 *
 * Основы вида `осуществля\p{L}*` в список не идут: модель прочитала бы такую
 * строку как текст, а не как узор, и написала бы её буквально.
 */
const BUREAUCRATIC_PLAIN = [
  'в рамках',
  'на данный момент',
  'в целях',
  'на основании',
...
/**
 * Обороты, которые называются словами и потому годятся для промпта, — по
 * группам, а не одним списком.
 *
 * Группами потому, что список у промпта конечный, а обрезать его подряд значит
 * выбросить целые правила: одни вводные слова заняли бы половину сорока строк,
 * и канцелярит модель не услышала бы вовсе. Кто берёт этот список, берёт его по
 * кругу — по одной фразе из группы, — и каждое правило оказывается названо.
 *
 * `negative-parallelism` — правило-узор, но сам ход настолько частый, что две
 * его канонические формы стоят здесь словами: промпту нужен пример, а не
 * регулярное выражение.
 */
export const RU_FORBIDDEN_PHRASE_GROUPS: readonly (readonly string[])[] = [
  ['не просто …, а …', 'не только …, но и …'],
  VVODNYE,
  TEMPLATE_TRANSITION,
  STOCK_OPENING,
  BUREAUCRATIC_PLAIN,
...
export const RU_FORBIDDEN_PHRASES: readonly string[] =
  RU_FORBIDDEN_PHRASE_GROUPS.flat();
...
export const RU_RULES: SlopRule[] = [
  ...
  },
  {
    ...
    severity: 'error',
    kind: 'regex',
    // Только пара целиком: «не просто X — это Y», «не только X, но и Y»,
    // «не просто X, а Y». Голое «не только» — обычные слова живого текста
    // («не только я так думаю»), и порог error на нём был шумом
    // (`content-factory-next-tu3k.8`, прогон владельца 06.09.2026).
    pattern: new RegExp(
      `${LEFT}не (?:просто|только)${RIGHT}[^.!?\\n]{0,80}?${LEFT}(?:но и|а|это)${RIGHT}` +
  ...
  },
  {
    ...
    severity: 'warn',
    kind: 'regex',
    pattern: phrases(PARTICIPLE_CLICHE),
    hint: {
  ...
  },
  {
    ...
    severity: 'warn',
    kind: 'regex',
    pattern: phrases(TEMPLATE_TRANSITION),
    hint: {
  ...
  },
  {
    ...
    pattern: phrases([
      ...
      'данная',
      'данное',
      // «данные» намеренно нет: как «data» это обычное слово живого текста
      // (`content-factory-next-tu3k.8`); канцелярское «данные меры» ловится
      // редко и не стоит ложных срабатываний на каждом «данные показали».
      `вышеупомянут${W}*`,
      `надлежащ${W}*`,
    ]),
    hint: {
