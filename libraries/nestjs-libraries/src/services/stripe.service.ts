...
export class StripeService {
  ...
  private async recordCancellation(
    ...
    try {
      await this._productEventsService.recordTrusted({
        ...
        properties: {},
        deduplicationKey: this.cancellationDeduplicationKey(
          subscriptionId,
          transitionId
        ),
        organizationId,
      ...
      // Analytics must not turn a completed billing transition into a failed
      // request: retrying setToCancel would reverse the cancellation. The
      // stable key still makes a later trusted replay safe.
      this._logger.warn(
        `Failed to record subscription cancellation for organization ${organizationId}`
      );
    }
  }

  private cancellationDeduplicationKey(
    subscriptionId: string,
    transitionId?: string
  ) {
    return transitionId
      ? `cancel_subscription:${subscriptionId}:${transitionId}`
      : `cancel_subscription:${subscriptionId}`;
  }

  async recordCancellationFromWebhook(
    event:
    ...
    if (!customerId) {
      // Terminal, like the two cases below. The billing mutation has already
      // committed by the time this runs, and no redelivery of the same event
      // will grow a customer field, so throwing would only make Stripe retry
      // for days and risk disabling the endpoint for every other event type.
      this._logger.warn(
        'Skipping cancellation event: Stripe customer id is missing'
      );
      return { recorded: false };
    }

    const organization =
      await this._organizationService.getOrgByCustomerId(customerId);
    if (!organization) {
      this._logger.warn(
        'Skipping cancellation event: Stripe customer has no organization'
      );
      return { recorded: false };
    }

    // Resolve an internal actor at delivery time. No local user identifier or
    // address is copied into Stripe metadata, and this query projects only the
    // id and role required to choose a stable owner.
    const actor = await this._organizationService.getProductEventActor(
      organization.id
    );
    if (!actor) {
      this._logger.warn(
        'Skipping cancellation event: organization has no event actor'
      );
      return { recorded: false };
    }

    return this._productEventsService.recordTrusted({
      name: 'cancel_subscription',
      properties: {},
      deduplicationKey: this.cancellationDeduplicationKey(
        event.data.object.id,
        event.data.object.metadata?.cancellationTransitionId
      ),
  ...
  async syncCustomerEmailsAfterSwitch(
    ...
    await Promise.all(
      [...emailByCustomer].map(([customerId, email]) =>
        stripe.customers
          .update(customerId, {
            email: email.indexOf('@') > -1 ? email : `${email}@content-factory.invalid`,
          })
          .catch(() => {})
      )
    );
  ...
  async createOrGetCustomer(organization: Organization) {
    ...
    const customer = await stripe.customers.create({
      email: users.users[0].user.email.indexOf('@') > -1 ? users.users[0].user.email : `${users.users[0].user.email}@content-factory.invalid`,
  ...
  async setToCancel(organizationId: string, userId: string) {
    const id = makeId(10);
    const org = await this._organizationService.getOrgById(organizationId);
    const customer = await this.createOrGetCustomer(org!);
    const currentUserSubscription = {
    ...
    if (sub.cancel_at_period_end) {
      const { cancel_at } = await stripe.subscriptions.update(sub.id, {
        ...
        metadata: {
          ...
          id,
          // Stripe removes metadata whose update value is empty. A later
          // cancellation gets a fresh transition id instead of inheriting the
          // event identity of the cancellation that was reversed.
    ...
    if (hasFailedPayment) {
      ...
      await stripe.subscriptions.update(sub.id, {
        metadata: {
          service: 'content-factory',
          cancellationTransitionId: id,
        },
      });
      await stripe.subscriptions.cancel(sub.id);
      await this._subscriptionService.deleteSubscription(customer);
      await this.recordCancellation(organizationId, userId, sub.id, id);
  ...
  private async createEmbeddedCheckout(
    ...
    try {
      await stripe.customers.update(customer, {
        email: user.email.indexOf('@') > -1 ? user.email : `${user.email}@content-factory.invalid`,
      });
    } catch (err) {}
  ...
  async cancelSubscription(organizationId: string, userId: string) {
    ...
    await stripe.subscriptions.update(subscriptions[0].id, {
      metadata: {
        service: 'content-factory',
        cancellationTransitionId: transitionId,
      },
    });
